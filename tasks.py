...
def coverage_html(c):
    ...
    """
    c.run("coverage run --source drf_impersonate runtests.py")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/drf-impersonate.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ drf_impersonate")
...
def lint(c):
    ...
    """
    c.run("flake8 drf-impersonate tests")
...
def release(c, bumpsize=""):
    ...
    import drf_impersonate

    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=drf_impersonate.__version__))
    c.run("git push --tags")
